...
/// ----------------------------------------------------------
/// $Id: fastqueue.c 6276 2011-06-17 07:18:20Z bless $
/// $HeadURL: https://svn.tm.kit.edu/nsis/protlib/trunk/fastqueue/fastqueue.c $
// ===========================================================
//                      
...
#ifdef QUEUELEN
      fprintf(stderr,"queue.c: length of queue (%s) grew up to %lu elements\n",
...
int signal_cond(queue_t *queuehead)
{
  if (queuehead != NULL)
  {
    if (pthread_mutex_lock(&queuehead->mutex)!=0)
