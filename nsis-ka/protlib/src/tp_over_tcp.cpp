...
/// ----------------------------------------------------------
/// $Id: tp_over_tcp.cpp 6148 2011-05-18 09:36:25Z bless $
/// $HeadURL: https://svn.tm.kit.edu/nsis/protlib/trunk/src/tp_over_tcp.cpp $
// ===========================================================
//                      
...
TPoverTCP::get_connection_to(const appladdress& addr)
{
  ...
  do 
  {
    ...
    // determine type, either IPv4 only or dual-stack socket
    int address_family= plibconf.getpar<bool>(protlibconf_ipv4_only) ? AF_INET : AF_INET6;

    new_socket = socket(address_family, SOCK_STREAM, IPPROTO_TCP);
    if (new_socket == -1)
    {
    ...
    {
	    addr.get_sockaddr(dest_address_v4);
    }
    ...
    socklen_t own_address_len= plibconf.getpar<bool>(protlibconf_ipv4_only) ? sizeof(own_address_v4) : sizeof(own_address);

    if (plibconf.getpar<bool>(protlibconf_ipv4_only))
	    getsockname(new_socket, reinterpret_cast<struct sockaddr*>(&own_address_v4), &own_address_len);
    else
	    getsockname(new_socket, reinterpret_cast<struct sockaddr*>(&own_address), &own_address_len);
    

    
    DLog(tpparam.name,">>--Connect-->> to " << addr.get_ip_str() << " port #" << addr.get_port() 
	 << " from " <<  ( plibconf.getpar<bool>(protlibconf_ipv4_only) ? 
						 inet_ntop(AF_INET,&own_address_v4.sin_addr,in6_addrstr,INET_ADDRSTRLEN) : 
						 inet_ntop(AF_INET6,&own_address.sin6_addr,in6_addrstr,INET6_ADDRSTRLEN) )
	 << " port #" << (plibconf.getpar<bool>(protlibconf_ipv4_only) ? ntohs(own_address_v4.sin_port) : ntohs(own_address.sin6_port)));
    ...
    assoc = new(nothrow) AssocData(new_socket, addr, 
				   ...
				   appladdress(own_address_v4,IPPROTO_TCP) : appladdress(own_address,IPPROTO_TCP) );
	...
	if (newmsg)
	{
		if (newmsg->send_to(tpparam.source) == false)
		{ // sending to own queue should not fail
			delete newmsg;
			ERRCLog(tpparam.name,"get_connection_to: couldn't find own queue " << tpparam.source);
		}
...
TPoverTCP::terminate(const address& in_addr)
{
 ...
 if (!addr) return;
 // since tpsend() converts to canonical format, we must do the same here
 ...
 * @param netmsg  - Data buffer that should be sent
 * @param in_addr - destination address
 * @param use_existing_connection - if true create a new connection if none exists, if false return error/failure
 * @param sentcallbackfunc - a function that is called if the message has been sent "successfully" - could be used for flow control
 * @note the netmsg is deleted by the send() method when it is not used anymore
 */
void
TPoverTCP::send_cb(NetMsg* netmsg, const address& in_addr, bool use_existing_connection, TPsentcallback_t sentcallbackfunc)
{
  if (netmsg == NULL) {
  ...
  if (destqueue)
  {
    ...
    TPoverTCPMsg* internalmsg= new TPoverTCPMsg(netmsg, addr->copy(), message::qaddr_unknown, sentcallbackfunc);
    if (internalmsg)
    {  
      // send the internal message to the sender thread queue
      if ( internalmsg->send(tpparam.source,destqueue) == false )
      { // send failed
 ...
 */
void
TPoverTCP::tcpsend(NetMsg* netmsg, appladdress* addr, TPsentcallback_t sentcallbackfunc)
{                        
#ifndef _NO_LOGGING
  ...
  if (addr) {
    // convert to canonical IPv6 address format
    addr->convert_to_ipv6();
  ...
  {
    // trigger sent callback function if one was specified and message was successfully sent down to socket buffer
    // note that this is no confirmation that the message arrived at the remote side, it is just intended to be
    // use for flow control feeback
    if ( sentcallbackfunc ) 
      (*sentcallbackfunc)(true);
...
TPoverTCP::sender_thread(void *argp)
{
  ...
  while (terminate==false && (internal_thread_msg= fq->dequeue()) != 0 )
  {
    ...
    if (internalmsg == 0)
    {
	    ERRLog(tpparam.name, methodname << "received not a TPoverTCPMsg but a " << internal_thread_msg->get_type_name());
    }
    else
	...
	try 
	{
		// send the NetMsg via TCP, address object is deleted inside tcpsend()
		tcpsend(internalmsg->get_netmsg(),internalmsg->get_appladdr(), internalmsg->get_sentcallback());
	} // end try
  ...
  */ 
  ...
  while( receiver_thread_argp->sig_terminate == false )
  {
	...
	else
	{
      ...
      {
	recv_error= true;
        ERRCLog(tpparam.name, methodname << "during receive buffer space exhausted");
      }
	...
	DLog(tpparam.name,"PDU debugging enabled - Received:" << hexdump.str());
      }
      ...
      {
	ERRLog(tpparam.name, methodname << "Cannot allocate/send TPMsg");
	if (tpmsg) delete tpmsg;
      } // end if tpmsg not allocated or not addr or not sent

      // prevent later double deletion on termination
      netmsg= NULL;
	...
	Log(DEBUG_LOG,LOG_NORMAL,tpparam.name,"Message too short to be a valid protocol header - dumping received bytes:" << hexdumpstr.str());	
      ...
      // free useless, empty, or partially filled netmsg
      delete netmsg;
      netmsg= NULL;
  ...
  DLog(tpparam.name, methodname << "Thread <" << pthread_self() 
  ...
  {
    if ( errno != ENOTCONN )
      ERRLog(tpparam.name, methodname << "Thread <" << pthread_self() << "> shutdown (read) on socket failed, reason: " << strerror(errno));
  }
  ...
  // delete any leftover network buffer
  delete netmsg;

  DLog(tpparam.name, methodname << "Thread <" << pthread_self() << "> terminated");

#ifdef _DEBUG
  DLog(tpparam.name, methodname << "Signaling main loop for cleanup");
#endif
...
TPoverTCP::stop_receiver_thread(AssocData* peer_assoc)
{
  ...
  if (recv_thread_argp)
  {
    if (!recv_thread_argp->terminated)
    {
      ...
      EVLog(tpparam.name,"stop_receiver_thread() - Receiver thread <" << thread_id << "> signaled for termination");
    ...
    ERRLog(tpparam.name,"stop_receiver_thread() - Receiver thread <" << thread_id << "> not found");
...
TPoverTCP::cleanup_receiver_thread(AssocData* peer_assoc)
{
  ...
  if (recv_thread_argp)
  {
    ...
    {
      // thread signaled termination, but is not?
      ERRLog(tpparam.name,"cleanup_receiver_thread() - Receiver thread <" << thread_id << "> not terminated yet?!");
      return;
    }
    else
    { // if thread is already terminated
      EVLog(tpparam.name,"cleanup_receiver_thread() - Receiver thread <" << thread_id << "> is terminated");
  ...
  DLog(tpparam.name,"cleanup_receiver_thread() - Cleanup receiver thread <" << thread_id << ">. Done.");
}
...
TPoverTCP::terminate_all_threads()
{
       ...
       terminate_iterator++)
  {
	...
	DLog(tpparam.name, "Signaled receiver thread <" << terminate_iterator->first << "> for termination");
	
	pthread_join(terminate_iterator->first, 0);
	
	DLog(tpparam.name, "Thread <" << terminate_iterator->first  << "> is terminated");
      }
      else
	DLog(tpparam.name, "Receiver thread <" << terminate_iterator->first << "> already terminated");
...
TPoverTCP::master_listener_thread()
{
  ...
  int master_listener_socket= -1;
  if (plibconf.getpar<bool>(protlibconf_ipv4_only))
	  master_listener_socket= socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
  else
	  master_listener_socket= socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP);
  ...
  if (bind_status)
    { 
      ERRCLog(tpparam.name, "Binding to " 
	      << (plibconf.getpar<bool>(protlibconf_ipv4_only) ? inet_ntop(AF_INET, &own_address_v4.sin_addr, in6_addrstr, INET6_ADDRSTRLEN) :
		  inet_ntop(AF_INET6, &own_address.sin6_addr, in6_addrstr, INET6_ADDRSTRLEN))
    ...
    if (listen_status)
    {
      ERRCLog(tpparam.name, "Listen at socket " << master_listener_socket 
	  << " failed, error: " << strerror(errno));
      return;
    }
    else
    {
      ILog(tpparam.name, color[green] << "Listening at port #" << tpparam.port << color[off]);
    }
    ...
    */
	...
	if (plibconf.getpar<bool>(protlibconf_ipv4_only))
		addr.set_ip(peer_address_v4.sin_addr);

	DLog(tpparam.name, "<<--Received connect--<< request from " << addr.get_ip_str() << " port #" << addr.get_port());
	...
	if (plibconf.getpar<bool>(protlibconf_ipv4_only))
		getsockname(conn_socket, reinterpret_cast<struct sockaddr*>(&own_address_v4), &own_address_len);
	else
	...
	AssocData* peer_assoc = new(nothrow) AssocData(conn_socket, addr,
						       plibconf.getpar<bool>(protlibconf_ipv4_only) ? 
						       appladdress(own_address_v4,IPPROTO_TCP) : appladdress(own_address,IPPROTO_TCP) );
	bool insert_success= false;
	if (peer_assoc)
	...
	{
	  ERRCLog(tpparam.name, "Cannot insert AssocData for socket " << conn_socket
...
TPoverTCP::~TPoverTCP()
{
  ...
  DLog(tpparam.name,  "Destructor called");
...
TPoverTCP::main_loop(uint32 nr)
{
  ...
  if (!fq) 
  {
    ERRCLog(tpparam.name, "Cannot find message queue");
    return;
  } // end if not fq
  ...
  while( currstate!=STATE_ABORT && currstate!=STATE_STOP )  
  {
	...
	if (internalmsg->get_msgtype() == TPoverTCPMsg::stop)
	{
	  ...
	  DLog(tpparam.name, "Got cleanup request for thread <" << assocd->thread_ID <<'>');
	  lock();
	...
	delete internalmsg;
      ...
      {
	ERRCLog(tpparam.name, "Dynamic_cast failed - received unexpected and unknown internal message source "
