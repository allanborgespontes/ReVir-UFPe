...
/// ----------------------------------------------------------
/// $Id: network_message.cpp 6148 2011-05-18 09:36:25Z bless $
/// $HeadURL: https://svn.tm.kit.edu/nsis/protlib/trunk/src/network_message.cpp $
// ===========================================================
//                     
 ...
 * @param s buffer size.
 */
	NetMsg::NetMsg(uint32 s) : buf(NULL), buf_len(0), pos(NULL), buf_end(NULL), allocated(true), tlp_list() {
 ...
 */
NetMsg::NetMsg(uchar *b, uint32 s, bool copy) : buf(NULL), buf_len(0), pos(NULL), buf_end(NULL), allocated(false), tlp_list(NULL) {
  if (s>max_size) throw NetMsgError(NetMsgError::ERROR_TOO_LONG);
  if (s==0) throw NetMsgError(NetMsgError::ERROR_INVALID_BUFSIZE);
  if (copy) {
    buf = new(nothrow) uchar[s];
...
// copy constructor
NetMsg::NetMsg(const NetMsg& n) : buf(NULL), buf_len(n.buf_len), pos(NULL), buf_end(NULL), allocated(false), 
				  tlp_list(n.get_TLP_list()?new TLP_list(*(n.get_TLP_list())):NULL) {
  // only copy a buffer of length > 0, otherwise we initialize as empty buffer
  if (buf_len)
  {
...
/** Frees the message buffer if memory was actually allocated. */
NetMsg::~NetMsg() {
  if (buf && allocated) { delete[] buf; buf= 0; }
  delete tlp_list;
} // end destructor
...
/** Copy n bytes into NetMsg buffer, starting from position 0
 * @param b source
...
/** Decode a uint16 integer. 
 @note buffer position is not changed! 	 */ 
uint16 NetMsg::decode16ro() const { 
	register uint16 i; 
	if ((pos+1)<=buf_end) { 
		i = ntohs(*((uint16*)pos)); 
	}  
...
void NetMsg::decode(uchar *c, uint32 len, bool move) {
  if (len == 0) // do nothing, otherwise next statement will fail
	  return;
...
TLP_list* NetMsg::get_TLP_list() 
{ 
    if (!tlp_list) 
	    tlp_list = new TLP_list();

    return tlp_list; 
}  

 	 
const TLP_list* NetMsg::get_TLP_list() const 
{  
  return tlp_list;  
}  
 
void NetMsg::fill_tlp_list(uint32 pdu_start, 
			   uint32 pdu_stop, 
			   uint32 (*get_length_from_obj_header)(const uint8*), 
			   uint16 (*get_type_from_obj_header)(const uint8*), 
			   uint16 (*get_subtype_from_obj_header)(const uint8*), 
			   bool clean_up_first) 
{ 
 	if (clean_up_first) {
		delete tlp_list;
	...
	if (!tlp_list) tlp_list= new TLP_list;
	
 	uint32 tmp_pos = pdu_start;
 	while ( tmp_pos < pdu_stop ) {
		uint8* tmp_adr = buf + tmp_pos;
		tlp_list->insert(get_type_from_obj_header(tmp_adr),get_subtype_from_obj_header(tmp_adr),tmp_pos);
		tmp_pos += get_length_from_obj_header(tmp_adr);
	}
} 
