...
/// ----------------------------------------------------------
/// $Id: ie.cpp 6148 2011-05-18 09:36:25Z bless $
/// $HeadURL: https://svn.tm.kit.edu/nsis/protlib/trunk/src/ie.cpp $
// ===========================================================
//                      
...
const char* IEError::err_str[] = {
	...
	"Object not properly aligned (wrong/missing padding)",
...
void IEManager::register_ie(uint16 category, uint16 type, uint16 subtype,
	...
	triple << "( Cat:" << category << ", Type:" << type << ", Subtype:" << subtype << ")";
	...
	if ( registry[key] != NULL ) {
		ERRLog("IEManager",
		       "An IE is already " << "registered for " << triple.str());
		return;
	}
	...
	try {
		...
		ERRLog("IEManager", "Cannot register IE for " << triple.str());

		throw IEError(IEError::ERROR_REGISTER);
	}

	// We made it so far, this means success.
	DLog("IEManager", "Registered IE [" << color[blue] << ie->get_ie_name() << color[off] << "] for " << triple.str());
}
