...
/// Transport over TCP
/// ----------------------------------------------------------
/// $Id: tp_over_tcp.h 6282 2011-06-17 13:31:57Z bless $
/// $HeadURL: https://svn.tm.kit.edu/nsis/protlib/trunk/include/tp_over_tcp.h $
// ===========================================================
//                      
...
public:
  ...
  virtual void send(NetMsg* msg, const address& addr, bool use_existing_connection, const address *local_addr) { send_cb(msg,addr,use_existing_connection); }
...
private:
  /// sends a network message, spawns receiver thread if necessary
  void send_cb(NetMsg* msg,const address& addr, bool use_existing_connection, TPsentcallback_t sentcallbackfunc= NULL);
  ...
  void tcpsend(NetMsg* msg, appladdress* addr, TPsentcallback_t sentcallbackfunc= NULL);
...
class TPoverTCPMsg : public message 
{
 ...
 private:
  ...
  NetMsg* netmsg;
  appladdress* addr;
  TPsentcallback_t sentcallbackfunc;

public:
  TPoverTCPMsg(const AssocData* peer_assoc, message::qaddr_t source= qaddr_unknown, TPoverTCPMsg::msg_t type= stop, TPsentcallback_t sentcallbackfunc= NULL ) : 
    message(type_transport, source), peer_assoc(peer_assoc), type(type), netmsg(0), addr(0), sentcallbackfunc(sentcallbackfunc) {}

  TPoverTCPMsg(NetMsg* netmsg, appladdress* addr, message::qaddr_t source= qaddr_unknown, TPsentcallback_t sentcallbackfunc= NULL) : 
    message(type_transport, source), peer_assoc(0), type(send_data), netmsg(netmsg), addr(addr), sentcallbackfunc(sentcallbackfunc) {}
  ...
  TPsentcallback_t get_sentcallback() const { return sentcallbackfunc; }
};
