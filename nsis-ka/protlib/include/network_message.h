...
/// ----------------------------------------------------------
/// $Id: network_message.h 6148 2011-05-18 09:36:25Z bless $
/// $HeadURL: https://svn.tm.kit.edu/nsis/protlib/trunk/include/network_message.h $
// ===========================================================
//                      
...
public:
	...
	NetMsg& to_start();
	/// Copy n bytes from buffer b into NetMsg buffer, starting there at position 0
	uint32 copy_from(const uchar *b, uint32 n);
	/// Copy n bytes from buffer b into NetMsg buffer, starting there at position start
	uint32 copy_from(const uchar *b, uint32 start, uint32 n);
	...
	/// decode uint16 read only 
	uint16 decode16ro() const;
	/// decode uint32
	...
	ostream& hexdump(ostream& os, uchar *startpos=0, uint32 length=0) const;
	 ...
	 /// clean up tlp_list 
	void clean_up_tlp_list() { delete tlp_list; tlp_list=NULL; } 
	/// get tlp_list 
	TLP_list* get_TLP_list();  
	const TLP_list* get_TLP_list() const; 
	/// fill TLP list from buffer 
	void fill_tlp_list(uint32 pdu_start, 
	              uint32 pdu_stop, 
...
private:
	...
	/// true if copied, used in destructor
	bool allocated;

	/// store type and position (in netmsg buffer)
	TLP_list* tlp_list;
