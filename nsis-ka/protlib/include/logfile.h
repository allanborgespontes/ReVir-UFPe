...
/// ----------------------------------------------------------
/// $Id: logfile.h 4948 2010-01-29 11:33:04Z stud-lenk $
/// $HeadURL: https://svn.tm.kit.edu/nsis/protlib/trunk/include/logfile.h $
// ===========================================================
//                      
...
extern volatile unsigned int stack_length;
...
#define  Log(lclass, llevel, mname, logstring) do {			\
  if ( protlib::log::DefaultLog.should_log(lclass, llevel) ) {		\
    ...
    string padding;							\
    if (stack_length != 0) padding.insert(0, stack_length*2, ' ');	\
    DefaultLog.logstart(lclass, llevel, mname) << padding << logstring;	\
    DefaultLog.logend();						\
  }									\
} while ( false )
...
#define  DEBUG_ENTER(mname, logstring) do {				\
  DLog(mname, "ENTER " << __FUNCTION__ << "() " << logstring);		\
  stack_length++;							\
} while ( false )

#define  DEBUG_LEAVE(mname, logstring) do {				\
  if (stack_length != 0) stack_length--;				\
  DLog(mname, "LEAVE " << __FUNCTION__ << "() " << logstring);		\
} while ( false )
...
#define  DEBUG_ENTER(mname, logstring)
#define  DEBUG_LEAVE(mname, logstring)
